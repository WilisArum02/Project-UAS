...
//        InputStream reportTemplate = Buku_1.class.getResourceAsStream("reportSkripsi.jrxml");
//        EntityManagerFactory emf = Persistence.createEntityManagerFactory("ProjectUASPU");
//        EntityManager em = emf.createEntityManager();
//        List<Skripsi_1> data = em.createQuery("SELECT s FROM Skripsi_1 s", Skripsi_1.class).getResultList();
...
try {
        ...
        switch (selection.toLowerCase()) {
            case "id_skripsi":
                queryString += "LOWER(s.id_skripsi) LIKE LOWER(:searchTerm)";
                break;
            case "judul":
            ...
            case "tahun":
                ...
                break;
            case "jumlah_halaman":
                queryString += "LOWER(s.jumlah_halaman) LIKE LOWER(:searchTerm)";
                break;
            default:
        ...
        Root<Skripsi_1> bookRoot = cq.from(Skripsi_1.class);
        cq.select(bookRoot);
        ...
        TypedQuery<Skripsi_1> q = em.createQuery(cq.where(cb.like(cb.lower(bookRoot.get(selection)), "%" + searchTerm.toLowerCase() + "%")));
        List<Skripsi_1> results = q.getResultList();
